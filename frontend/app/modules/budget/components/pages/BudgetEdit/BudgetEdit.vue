<script setup lang="ts">
    ...
    import { fetchBudget } from '~/modules/budget/domain/api/bugdet/fetchBudget';
    import { updateBudget } from '~/modules/budget/domain/api/bugdet/updateBudget';
    ...
    const itemState = ref<IBudgetItemState | null>(null);
    const itemObject = ref<IBudgetItem | null>(null);
    ...
    const fetchItem = async (): Promise<IBudgetItem | null> => {
        isLoading.value = true;
        try {
            const data = await fetchBudget(props.id);
            return data.item;
        } catch (e) {
    ...
    const updateItemState = (item: IBudgetItem) => {
        itemObject.value = item;

        const stateValue: IBudgetItemState = {
            amount: item.amount,
            period: item.period,
    ...
    const save = async () => {
        ...
        isLoading.value = true;
        try {
            await updateBudget(itemObject.value.id, {
                ...itemState.value,
            });

...
func (r *Repository) Create(ctx context.Context, item *entity.Transaction) error {
	const op = "Create"
	...
	query, args, err := r.qb.Insert(pg.TransactionTable).SetMap(dataMap).ToSql()
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, item *entity.Transaction) error {
	const op = "Update"
	...
	err = r.pgClient.Do(ctx, func(ctx context.Context) error {
		...
		updQuery, updArgs, err := r.qb.Update(pg.TransactionTable).Where(
